...
# =============================
def build_network(num_actions, agent_history_length, resized_width, resized_height,reuse=False,variablename='g'):
  #with tf.device("/cpu:0"):

  with tf.variable_scope(variablename, reuse=reuse):
    tl.layers.set_name_reuse(reuse)
    state = tf.placeholder("float", [None, agent_history_length, resized_width, resized_height])
    network = tl.layers.InputLayer(state,name='input')
    ...
    network = tl.layers.DenseLayer(network,n_units=num_actions, act=tf.identity,name='out')
    m = network.outputs
  return state, m
...
class AtariEnvironment(object):
    ...
    def __init__(self, gym_env, action_repeat):
        ...
        self.gym_actions = xrange(gym_env.action_space.n)
    ...
    def get_initial_state(self):
        ...
        s_t = np.stack([x_t for i in xrange(self.action_repeat)], axis=0)

        for i in xrange(self.action_repeat-1):
    ...
    while T < TMAX:
        ...
        while True:
            ...
            if t % checkpoint_interval == 0:
                saver.save(session, "qlearning.ckpt")
...
def build_graph(num_actions):
    ...
    s, q_network = build_network(num_actions=num_actions, agent_history_length=4, resized_width=84, resized_height=84,reuse=False,variablename='a')
    network_params = tf.trainable_variables()
    q_values = q_network

    # Create shared target network
    st, target_q_network = build_network(num_actions=num_actions, agent_history_length=4, resized_width=84, resized_height=84,reuse=False,variablename='b')
...
def train(session, graph_ops, num_actions, saver):
    ...
    envs = [gym.make(game) for i in xrange(n_threads)]
    ...
    try:
        saver.restore(session, "qlearning.ckpt")
    except Exception:
