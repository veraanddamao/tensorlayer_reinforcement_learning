...
from a3c_model import build_policy_networks
...
ACTIONS = gym.make(GAME).action_space.n
#ACTIONS = 5
...
def actor_learner_thread(num, env, session, graph_ops, summary_ops, saver):
    ...
    while T < TMAX:
        ...
        R_batch = np.zeros(t)
        for i in reversed(xrange(t_start, t)):
...
def build_graph():
    ...
    s, p_network, v_network = build_policy_networks(num_actions=ACTIONS, agent_history_length=AGENT_HISTORY_LENGTH, resized_width=RESIZED_WIDTH, resized_height=RESIZED_HEIGHT,reuse=False)
...
def train(session, graph_ops, saver):
    ...
    envs = [gym.make(GAME) for i in xrange(NUM_CONCURRENT)]
    ...
    if goon:
        saver.restore(session, 'haha.ckpt')
        print 'Load ok!'
    else:
        print 'Creat new!'
